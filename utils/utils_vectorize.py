...
# Matrix similarity
# Bidirectional average of maximum similirity
def bidir_avgmax_sim(similarity_matrix, stdst='mean', kg0norms=[], kg1norms=[]):
...
# BAMS with term specificity weighting, using norms as an approximation for IDF
def weighted_bams_sim(similarity_matrix, norms0, norms1):
    ...
    for i in range(n_rows):
        max_sim = np.max(sim_matrix[i, :]) #if n_cols > 0 else 0
        idx_max = np.argmax(sim_matrix[i, :])
        from_rows_op += max_sim * norms1[idx_max] / sum(norms0)
    from_cols_op = 0.
    for j in range(n_cols):
        max_sim = np.max(sim_matrix[:, j]) #if n_rows > 0 else 0
        idx_max = np.argmax(sim_matrix[:, j])
        from_cols_op += max_sim * norms0[idx_max] / sum(norms1)
    '''
    from_rows_op = 0.
    for i in range(n_rows):
        max_sim = np.max(sim_matrix[i, :]) #if n_cols > 0 else 0
        from_rows_op += max_sim * norms0[i] / sum(norms0)
    from_cols_op = 0.
    for j in range(n_cols):
        max_sim = np.max(sim_matrix[:, j]) #if n_rows > 0 else 0
        from_cols_op += max_sim * norms1[j] / sum(norms1)

    return((from_rows_op + from_cols_op) / 2)
...
# Mihalcea tests from a pair of texts/sentences (norms as an approximation for IDF)
def mihalcea_sim(txt_pair, stop_words=True, punct_marks=False, embed_model='w2v'):
    if embed_model == 'w2v': model = w2v_model
    if embed_model == 'glove': model = glove_model
    s1_v = []
    s2_v = []
    s1 = _clean_txt(txt_pair[0], stop_words=stop_words, punct_marks=punct_marks).split()
    s2 = _clean_txt(txt_pair[1], stop_words=stop_words, punct_marks=punct_marks).split()
    for elem in s1:
        try:
            token = model[elem]
        except:
    ...
    for elem1 in s1_v:
        sims = []
        norm = np.linalg.norm(elem1)
        for elem2 in s2_v:
            sim = cos_sim(elem1, elem2)
            sims.append(sim)
        max_sim = np.max(sims)
        sum_max_sims += max_sim * norm
        sum_norms += norm
    s1_op = sum_max_sims / sum_norms
    
    sum_max_sims = 0
    sum_norms = 0
    for elem2 in s2_v:
        sims = []
        norm = np.linalg.norm(elem2)
        for elem1 in s1_v:
            sim = cos_sim(elem2, elem1)
            sims.append(sim)
        max_sim = np.max(sims)
        sum_max_sims += max_sim * norm
        sum_norms += norm
    s2_op = sum_max_sims / sum_norms
    
    return (s1_op + s2_op) / 2 
